...
vector <char> read_file(string inputFile) {
	...
	if (Myfile) {
...

		{
			...
			arrayout.push_back(my_character);
	...
	for (int i = 0; i < size; i++) {
		...
		{	
			...
			if (index == -1) {
				...
				newSize ++ ;//for character frequency
...

		 {
			 ...
			 cout << characterFrequency[i].letter << " ==> " << characterFrequency[i].count << endl;

		 }*/

		print_characterFrequency(characterFrequency, newSize);
